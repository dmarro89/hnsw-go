...
def run(cmd,cwd=ROOT): subprocess.run(cmd,cwd=cwd,check=True,text=True)
def dataset():
 ...
 return p
def build(ref,name):
 wd=ART/f'wt-{name}'; run(['git','worktree','add','--detach',str(wd),ref])
 try: run(['go','build','-o',str(ART/f'runner-{name}'),'./benchmarks/crosslang/go'],wd)
 finally: run(['git','worktree','remove','--force',str(wd)])
