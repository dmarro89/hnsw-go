...
func (h *HNSW) insertBatchArenaLocked(vectors [][]float32, dim int) {
	...
	neighborArena := newBulkNeighborArena(h, len(vectors))
	searchBuf := make([]int, 0, h.EfConstruction)
	for id := range vectors {
		start := id * dim
		vector := arena[start : start+dim]
		searchBuf = h.insertLockedArenaWithLevel(vector, id, neighborArena.levels[id], searchBuf, arena, dim, neighborArena)
	}
}
...
func (h *HNSW) insertLockedArenaWithLevel(vector []float32,id,level int,searchBuf []int,arena []float32,dim int,neighborArena *bulkNeighborArena)[]int{
	newNode:=h.appendNodeBulkArena(id,vector,level,neighborArena)
	...
	for lc:=int(math.Min(float64(L),float64(level)));lc>=0;lc--{
		...
		maxConn:=h.Mmax;if lc==0{maxConn=h.Mmax0}; neighbors:=nearest[:min(len(nearest),h.M)]
...
func (h *HNSW) searchLayerArena(query []float32,entries []int,ef,level int,dst []int,arena []float32,dim int)[]int{h.visitStamp++;stamp:=h.visitStamp;pool:=h.ensureHeapPool();candidates:=pool.GetMinHeap();defer pool.PutMinHeap(candidates);nearest:=pool.GetMaxHeap();defer pool.PutMaxHeap(nearest);for _,id:=range entries{if id<0||id>=len(h.Nodes)||h.markVisitedArena(id,stamp){continue};d:=h.DistanceFunc(query,arenaVector(arena,dim,id));candidates.Push(structs.NewNodeHeap(d,id));nearest.Push(structs.NewNodeHeap(d,id));if nearest.Len()>ef{nearest.Pop()}};if candidates.Len()==0{return dst[:0]};for candidates.Len()>0{current:=candidates.Pop();if nearest.Len()>=ef&&current.Dist>nearest.Peek().Dist{break};node:=h.Nodes[current.Id];if node==nil||level>=len(node.Neighbors){continue};for _,id:=range node.Neighbors[level]{if id<0||id>=len(h.Nodes)||h.markVisitedArena(id,stamp){continue};d:=h.DistanceFunc(query,arenaVector(arena,dim,id));if nearest.Len()>=ef&&d>=nearest.Peek().Dist{continue};candidates.Push(structs.NewNodeHeap(d,id));nearest.Push(structs.NewNodeHeap(d,id));if nearest.Len()>ef{nearest.Pop()}}};n:=nearest.Len();if cap(dst)<n{dst=make([]int,n)}else{dst=dst[:n]};for i:=n-1;i>=0;i--{dst[i]=nearest.Pop().Id};return dst}
